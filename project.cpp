...
std::string AES_Encrypt(const std::string& fileContent, const std::string& inputKey){
    ...
    for (int i = 0; i < lenOfPaddedMessage; i++){
        ...
        std::cout << std::hex << (int)(unsigned char)(encryptedFile[i]) << ' ';
    }
...
void InverseSubBytes(unsigned char* state) {
    for (int i = 0; i < 16; i++) {
        ...
        //std::cout << std::hex << (unsigned int)state[i] << ' ' << (unsigned int)InverseSBox[state[i]] << std::endl;
    }
}
...
int main(){
    ...
    std::string skey = "abcdefghijklmnop";
